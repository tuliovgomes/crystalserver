local config = {
    ...
    storage_quest = 50003 -- storage para controle da quest
}

local player_positions = {
    [1] = {fromPos = Position(770, 708, 6), toPos = Position(698, 791, 7)},
    [2] = {fromPos = Position(771, 708, 6), toPos = Position(698, 791, 7)},
    [3] = {fromPos = Position(772, 708, 6), toPos = Position(698, 791, 7)},
    [4] = {fromPos = Position(773, 708, 6), toPos = Position(698, 791, 7)}
}

local monsters = {
    [1] = {pos = Position(698, 783, 7), name = "Valaraukar"},
}

function doResetFidelitas(uid)
    local item = Item(uid)
    if not item then
        return
    end
...
local Fidelitas = Action()

function Fidelitas.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    addEvent(doResetFidelitas, config.duration * 60 * 1000, item.uid)
    return true
end
